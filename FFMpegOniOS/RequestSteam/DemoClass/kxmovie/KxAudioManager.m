...
- (BOOL)isHeadsetPluggedIn
{
    ...
    {
        headphonesLocated |= ( [portDescription.portType isEqualToString:AVAudioSessionPortHeadphones] );//插线耳机
        //        headphonesLocated |= ( [portDescription.portType isEqualToString:AVAudioSessionPortBluetoothA2DP] );//蓝牙输出设备，可能是耳机或者蓝牙音箱
    }
...
//                    error: &error];
    ...
    if (!success) {
        LoggerAudio(1, @"Error: Couldn't set audio category.");
        return NO;
    }
...
//    }
    ...
    if(error)
    {
        LoggerAudio(2, @"Couldn't set the preferred buffer duration");
    }
...
//        return NO;
    ...
    if (error) {
        LoggerAudio(2,@"Couldn't activate the audio session");
         return NO;
    }
...
//        return NO;
    ...
    newNumChannels = [AVAudioSession sharedInstance].outputNumberOfChannels;
...
//                   "Couldn't deactivate the audio session");
        ...
        if (error) {
            LoggerAudio(2,@"Couldn't deactivate the audio session");
        }
        //--------------------------------------------------------------------------------------
...
- (void)interruptionNotificationCallback:(NSNotification *)notification
{
    ...
    NSInteger inInterruption = [[interuptionDict valueForKey:AVAudioSessionInterruptionTypeKey] integerValue];
...
- (void)audioRouteChangeListenerCallback:(NSNotification *)notification
{
    ...
    switch (routeChangeReason) {
        ...
        case AVAudioSessionRouteChangeReasonCategoryChange:
            ...
            LoggerAudio(1,@"AVAudioSessionRouteChangeReasonCategoryChange");
            break;
    }
...
{
//    Printing description of interuptionDict:
//    {
...
//        "AVSystemController_AudioVolumeNotificationParameter" = "0.0625";
...
//    [self.view addSubview: volumeView];
    ...
    NSDictionary *interuptionDict = notification.userInfo;
    float volume = [[interuptionDict objectForKey:@"AVSystemController_AudioVolumeNotificationParameter"] floatValue];
    LoggerAudio(1,@"volumn is %f", volume);
    self.outputVolume = volume;
    NSLog(@"%f",self.outputVolume);
...
//            
//            float sad = *(float *)inData;
//            
//            sm.outputVolume = *(float *)inData;
//            NSLog(@"%f",sm.outputVolume);
//
//        }
//    }
//}
