...
static void avStreamFPSTimeBase(AVCodecContext *vCodecCtx,AVStream *st, CGFloat defaultTimeBase, CGFloat *pFPS, CGFloat *pTimeBase)
{
    ...
    else if(vCodecCtx->time_base.den && vCodecCtx->time_base.num)
        timebase = av_q2d(vCodecCtx->time_base);
    else
        timebase = defaultTimeBase;
        
    if (vCodecCtx->ticks_per_frame != 1) {
        LoggerStream(0, @"WARNING: st.codec.ticks_per_frame=%d", vCodecCtx->ticks_per_frame);
...
//        if (codecType == formatCtx->streams[i]->code->codec_type)
...
//                [md setValue: [NSNumber numberWithInt:_formatCtx->bit_rate]
            ...
            if (_videoStreams.count) {
                ...
                for (NSNumber *n in _videoStreams) {
                    ...
                    AVCodecParameters *p_codec_parameters = st->codecpar;
                    AVCodec *p_codec = avcodec_find_decoder(p_codec_parameters->codec_id);
                    AVCodecContext *p_codec_context = avcodec_alloc_context3(NULL);
...
//                    avcodec_string(buf, sizeof(buf), st->codec, 1);
                    avcodec_string(buf, sizeof(buf), p_codec_context, 1);
...
//                    avcodec_string(buf, sizeof(buf), st->codec, 1);
                    if (n.integerValue == _videoStream) {
...
//    AVCodec *codec = avcodec_find_decoder(codecCtx->codec_id);
    ...
    AVCodecParameters *p_codec_parameters = _formatCtx->streams[videoStream]->codecpar;
    AVCodec *codec = avcodec_find_decoder(p_codec_parameters->codec_id);
    AVCodecContext *codecCtx = avcodec_alloc_context3(NULL);
    avcodec_parameters_to_context(codecCtx, p_codec_parameters);
    ...
    AVStream *st = _formatCtx->streams[_videoStream];
    avStreamFPSTimeBase(codecCtx,st, 0.04, &_fps, &_videoTimeBase);
...
- (kxMovieError)openAudioStream:(int)audioStream
{   
//    AVCodecContext *codecCtx = _formatCtx->streams[audioStream]->codec;
//    AVCodec *codec = avcodec_find_decoder(codecCtx->codec_id);
    AVCodecParameters *p_codec_parameters = _formatCtx->streams[audioStream]->codecpar;
    AVCodec *codec = avcodec_find_decoder(p_codec_parameters->codec_id);
    AVCodecContext *codecCtx = avcodec_alloc_context3(NULL);
    avcodec_parameters_to_context(codecCtx, p_codec_parameters);
    ...
    AVStream *st = _formatCtx->streams[_audioStream];
    avStreamFPSTimeBase(codecCtx,st, 0.025, 0, &_audioTimeBase);
...
//    AVCodec *codec = avcodec_find_decoder(codecCtx->codec_id);
    ...
    AVCodecParameters *p_codec_parameters = _formatCtx->streams[subtitleStream]->codecpar;
    AVCodec *codec = avcodec_find_decoder(p_codec_parameters->codec_id);
    AVCodecContext *codecCtx = avcodec_alloc_context3(NULL);
    avcodec_parameters_to_context(codecCtx, p_codec_parameters);
...
//                                                &packet);
                ...
                int ret = avcodec_send_packet(_videoCodecCtx, &packet);
...
//                                                &packet);
                ...
                int ret = avcodec_send_packet(_audioCodecCtx, &packet);
