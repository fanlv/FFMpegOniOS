    ...
    if (audio_index >= 0) {
        ...
        switch (pAudioCodecCtx->codec_id) {
            ...
            case AV_CODEC_ID_PCM_U8:
                ...
                break;
            default:
                NSLog(@"Error: audio format '%s' (%d) is not supported", pAudioCodecCtx->codec->long_name, pAudioCodecCtx->codec_id);
...
//            audioFormat.mFormatFlags = kAudioFormatFlagIsFloat | kAudioFormatFlagsNativeEndian | kAudioFormatFlagIsPacked;
            ...
            if ((err = AudioQueueNewOutput(&audioFormat, HandleOutputBuffer, (__bridge void *)(self), NULL, NULL, 0, &mQueue)) != noErr) {
                ...
                vBufferSize = [self DeriveBufferSize:audioFormat withPakcetSize:(UInt32)pAudioCodecCtx->bit_rate/8 andSeconds:AUDIO_BUFFER_SECONDS];
...
- (void)play
{
    ...
    if (err != noErr) {
        NSLog(@"AudioQueue Start error %d", (int)err);
    }
